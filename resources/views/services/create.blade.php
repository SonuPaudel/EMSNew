@extends('layouts.app')
@section('content')
<h2 class="text-5xl font-bold mt-5  pt-10 text-black ">Add Service</h2>
<div class=" my-10  w-9/12 h-8/12 border-spacing-2 border-2 bg-slate-200">
    ...
    <form action="{{ route('services.store') }}" method="POST" enctype="multipart/form-data" class="ml-5 mr-5 mt-2">
        @csrf
        <input type="text" class="w-full p-2 rounded-lg mt-2" name="name" placeholder="Enter Name" value="{{old('name')}}">
        @error('name')
        <span class="text-red-500 -mt-4">*{{$message}}</span>
        @enderror

        <input type="text" class="w-full p-2 rounded-lg mt-2" name="type" placeholder="Enter Type" value="{{old('type')}}">
        @error('type')
        <span class="text-red-500 -mt-4">*{{$message}}</span>
        @enderror

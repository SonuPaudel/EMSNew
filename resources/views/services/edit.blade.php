...
<div class="my-10 w-11/12 md:w-9/12 lg:w-8/12 mx-auto border-2 bg-slate-200 rounded-lg">
    <h2 class="text-xl font-bold text-black p-2 bg-slate-300">Service Detail</h2>
    <form action="{{ route('services.update', $services->id) }}" method="POST" enctype="multipart/form-data" class="px-6 py-4">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="name" class="w-full p-2 rounded-lg mt-2" name="name" placeholder="Service Name" value="{{ $services->name }}">
            @error('name')
            <span class="text-red-500 mt-1">* {{ $message }}</span>
            @enderror
        </div>
        <div class="mb-4">
            <label for="event_id" class=" text-gray-700 ">Choose Event Type</label>

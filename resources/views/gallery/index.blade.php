    ...
    <table id="example" class="display">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$gallery->name}}</td>
                <td>
                    <div class="h-20 overflow-auto">{!!$gallery->description!!}</div>
                </td>
                <td><img class="w-20" src="{{asset('images/gallery/'.$gallery->photopath)}}" alt=""></td>
                <td>
                    <a href="{{ route('gallery.edit',$gallery->id)}}" class="bg-blue-600 text-white px-4 py-1 rounded-lg mx-1 cursor-pointer  hover:bg-blue-800">Edit</a>
                    <a onclick="showDelete('{{$gallery->id}}')" class="bg-red-600 text-white px-4 py-1 rounded-lg mx-1 cursor-pointer  hover:bg-red-800">Delete</a>
                </td>
            </tr>
            @endforeach
        </tbody>

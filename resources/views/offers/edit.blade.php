@extends('layouts.app')
@section('content')
<h2 class="text-4xl font-bold mt-5 pt-10 text-black">Edit Offer</h2>
    ...
    <form action="{{ route('offers.update', $offer->id) }}" method="POST" class="px-6 py-4">
        ...
        <div class="mb-4">
            ...
            <select class="w-full p-2 rounded-lg mt-2" name="service_id" id="service_id">
                ...
                <option value="{{ $service->id }}" {{ $offer->service_id == $service->id ? 'selected' : '' }}>{{ $service->name }}</option>
                @endforeach
            </select>
            @error('service_id')
        ...
        <div class="mb-4">
            ...
            <input type="text" class="w-full p-2 rounded-lg mt-2" id="discounted_rate" name="discounted_rate" placeholder="Enter Discounted Rate" value="{{ old('discounted_rate', $offer->discounted_rate) }}">

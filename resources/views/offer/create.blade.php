...
<div class=" my-10 w-9/12 h-8/12 border-spacing-2 border-2 bg-slate-200">
    ...
    <form action="{{ route('offer.store') }}" method="POST" enctype="multipart/form-data" class=" ml-5 mr-5 mt-2">
        ...
        <input type="text" id="rate" class="w-full p-2 rounded-lg mt-2" name="rate" placeholder="Enter Rate" value="{{old('rate')}}">
        @error('rate')
        <span class="text-red-500 -mt-4">*{{$message}}</span>
        @enderror
        <input type="text" class="w-full p-2 rounded-lg mt-2" name="capacity" placeholder="Enter Capacity" value="{{old('capacity')}}">
        @error('capacity')
        <span class="text-red-500 -mt-4">*{{$message}}</span>
        @enderror
        ...
        <div class=" mt-10  mb-10 text-center">
            ...
            <a href="{{ route('offer.index') }}" class="bg-red-600 text-white px-4 py-1.5 rounded-lg cursor-pointer hover:bg-red-800">Exit</a>
        </div>

...
<div class=" my-10  w-9/12 h-8/12 border-spacing-2 border-2 bg-slate-200">
    ...
    <form action="{{route('venue.store')}}" method="POST" class=" ml-5 mr-5 mt-2" enctype="multipart/form-data">
        @csrf
        <input type="text" class="w-full p-2 rounded-lg mt-2" name="name" placeholder="Enter Venue-name" value="{{old('name')}}">
        @error('name')
        <span class="text-red-500 -mt-4">* {{$message}}</span>
        @enderror

        <input type="text" class="w-full p-2 rounded-lg mt-2" name="location" placeholder="Enter location" value="{{old('location')}}">
        @error('location')
        <span class="text-red-500 -mt-4">* {{$message}}</span>
        @enderror
